...
| 快捷键 | 说明 |
| --- | --- |
| `<Leader>os` | 选择 file snippet（`all-snippets/oi-snippets/files/`） |
...
Snippet 按用途拆在 `all-snippets/lua-snippets/` 下（C++ 入口 `cpp.lua`、Python 入口 `python.lua`，
...
这些 snippets 同时由 Neovim 和 VSCode 从 `all-snippets/vscode-snippets/python.json` 加载。
